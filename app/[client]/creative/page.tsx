...
import { COHORTS, ADS, getCreativeChurnData } from "@/lib/mock-data";
...
import { CohortMetric } from "@/lib/types";
...
export default function CreativePage() {
  ...
  const [gallerySort, setGallerySort] = useState<GallerySort>("hookRate");

  const cohorts = COHORTS[clientId] ?? [];
  const ads = ADS[clientId] ?? [];
  const churnData = getCreativeChurnData(clientId);
          ...
          ads.length > 0 ? (
            ...
            <div className="rounded-xl border border-[var(--color-border)] p-12 text-center">
              <p className="text-sm text-[rgba(9,10,8,0.4)]">
                Annonse-galleri kobles til Meta API i neste fase.
              </p>
            </div>
          )
        )}
